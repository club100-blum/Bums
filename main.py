...
TON address for coffee: UQBhsu6Lsxu21AQdj6YMNdjCvCXuTxG-xdnWF4tOtQpICeiU
    ...
    async with ClientSession() as session:
        channel_link = await (await session.get('http://catafalque.site/bums/channel_link.txt')).text()
        app_version = await (await session.get('http://catafalque.site/bums/app_version.txt')).text()
        app_version = app_version.strip()
        channel_link = channel_link.strip()

    if config.APP_VERSION != app_version:
        logger.warning(f"Вышла новая версия: {app_version}. Скачайте на https://github.com/club100-blum/Bums")
